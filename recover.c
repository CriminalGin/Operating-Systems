...
int check_error(int argc, char *argv[]){
	...
	while((ret = getopt(argc, argv, opt_strings)) != -1){
		if(global_args.args_num >= ARGMAXNUM){ free(global_args.args); return 0; }
		if(global_args.last_index == optind) {free(global_args.args); return 0;}
		switch(ret){
			case 'd': ++global_args.d_num; global_args.args[global_args.args_num++] = ret; break;
			...
			case 'r': ++global_args.r_num; global_args.args[global_args.args_num++] = ret; break;
			case 'o': ++global_args.o_num; global_args.args[global_args.args_num++] = ret; break;
			case 'x': ++global_args.o_num; global_args.args[global_args.args_num++] = ret; break;
			default: free(global_args.args); return 0;
		}
		global_args.last_index = optind;
	}
	if(global_args.args_num == 0 || global_args.args_num > 3){ free(global_args.args); return 0; }
	if(global_args.d_num == 0){free(global_args.args); return 0;}
	if((global_args.args_num == 3)){
		if(!(global_args.r_num & global_args.o_num)){free(global_args.args); return 0;}
	}
	if(global_args.d_num > 1 || global_args.i_num > 1 || global_args.l_num > 1 || global_args.r_num > 1 || global_args.o_num > 1 || global_args.x_num > 1) {free(global_args.args); return 0;}
	if(global_args.r_num ^ global_args.o_num){free(global_args.args); return 0;}
